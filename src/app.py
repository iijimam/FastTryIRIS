...
with col2:
    if st.button("データベースから履歴読込"):
        ...
        if not retjson:
            ...
            #latest_conv = retjson[0]
            st.session_state.messages_model = []  # 置き換えたいなら初期化
            for latest_conv in retjson:
                # 念のため型をチェック
                if isinstance(latest_conv, list) and all(isinstance(m, dict) for m in latest_conv):
                    st.session_state.messages_model.extend(latest_conv)  # ←ここがポイント（appendじゃなくextend）
                    st.session_state.messages_view.extend(latest_conv) #表示用にも追加
                else:
                    st.error("DBからのデータ形式が想定外でした（List[Dict]ではありません）。")        

            #ここまででmessage_viewの中に role:userがない時、直近の要約を表示しておく
            if any(d.get("role")!="user" for d in st.session_state.messages_view):
                latest=st.session_state.messages_view[len(st.session_state.messages_view)-1]
                latestsummary=latest["content"].split("SummaryHistory:")[1].strip()
                print(latestsummary)
                st.session_state.messages_view.extend([{"role":"user","content":f"【過去の問い合わせ要約】\n\n {latestsummary}"}])
